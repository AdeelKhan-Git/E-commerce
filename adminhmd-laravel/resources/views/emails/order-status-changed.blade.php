...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;">
    ...
    <tr>
        <td style="padding:30px;">
            ...
            <p style="color:#555;">Your order <strong>{{ $order->order_number }}</strong> has an update.</p>

            <table width="100%" style="background:#f9f9f9;border-radius:8px;padding:20px;margin:20px 0;text-align:center;border-collapse:collapse;">
                <tr>
                    <td style="padding:10px;">
                        <p style="color:#999;margin:0 0 8px;font-size:13px;text-transform:uppercase;letter-spacing:1px;">{{ $label }}</p>
                        <span style="background:{{ $bg }};color:{{ $fg }};padding:8px 20px;border-radius:20px;font-weight:bold;font-size:16px;">
                            {{ ucfirst($newStatus) }}
                        </span>
                        <p style="color:#aaa;margin:10px 0 0;font-size:12px;">Previously: {{ ucfirst($oldStatus) }}</p>
                    </td>
                </tr>
            </table>
            ...
            <table width="100%" style="border-collapse:collapse;margin:20px 0;">
                ...
                @endforeach
                <tr>
                    <td colspan="2" style="padding:15px 0 0;text-align:right;font-weight:bold;">Order Total:</td>
                    <td style="padding:15px 0 0;text-align:right;font-weight:bold;color:#00a8b3;">${{ number_format($order->total_amount, 2) }}</td>
                </tr>
            </table>

            <p style="color:#555;">If you have any questions about this update, feel free to reply to this email.</p>
        </td>
    </tr>
