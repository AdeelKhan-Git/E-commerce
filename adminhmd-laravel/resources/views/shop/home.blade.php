...
<section style="padding:10px 0 60px;">
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="{{ route('shop') }}" class="btn-primary-custom" style="font-size:13px;padding:8px 20px;">View All</a>
        </div>
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="section-title mb-0" style="color:var(--stormy-teal);">
                <i class="bi bi-fire me-2" style="color:var(--tangerine-dream);"></i>Hot <span style="color:var(--tangerine-dream);">Deals</span>
            </h2>
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <div class="product-card" style="border-color:rgba(226,149,120,.4);">
                    ...
                    <div class="product-card-body">
                        ...
                        <a href="{{ route('product.show', $product) }}" class="product-name">{{ $product->product_name }}</a>
                        <div class="product-price mb-2" style="color:var(--tangerine-dream);">${{ number_format($product->price, 2) }}</div>
                        @auth
                            <form method="POST" action="{{ route('cart.add') }}">
                                @csrf
                                <input type="hidden" name="product_id" value="{{ $product->id }}">
                                <input type="hidden" name="quantity" value="1">
                                <button type="submit" class="btn-add-cart" style="background:var(--tangerine-dream);">
                                    <i class="bi bi-cart-plus me-1"></i>Add to Cart
                                </button>
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="btn-add-cart d-block text-center text-decoration-none" style="background:var(--tangerine-dream);">
                                <i class="bi bi-cart-plus me-1"></i>Add to Cart
                            </a>
                        @endauth
                    </div>
